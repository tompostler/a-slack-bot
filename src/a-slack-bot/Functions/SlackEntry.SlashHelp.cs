...
A bot constructed for the express purpose of relieving boredom and maybe even providing some useful functionality.
Interaction with the bot is mostly random with numerous easter eggs and various nuggets of goodness thrown in.
...
The following are slash commands that can be triggered by the app:
    ...
    /asb-help           Displays this helptext.
    /asb-reaction (add|list|remove) ['key' [value]]
                        Add, list or remove custom message reactions. Sending the command with no arguments will get you
                        additional helptext.
    /asb-response (add|list|remove) ['key' [value]]
                        Add, list or remove custom message responses. Sending the command with no arguments will get you
                        additional helptext.
    /asb-send-as-me [t] Store a user token to be used for replying to slash commands as you instead of as the bot. This
                        uses Slack's legacy token feature because I don't want to deal with the proper OAuth 2.0 Slack
                        flow. If you wish to remove token, pass 'clear' for the token value. Visit
    ...
    /asb-whitelist (add|remove) 'thing-to-whitelist'
                        Based on the thing-to-whitelist, will update whatever is necessary to make it happen. An example
                        right now is needing to whitelist the /blackjack command in a channel.
    ...
    /guess [num]        Play a single-number guessing game where the number has to be in the interval (0, balance].
                        Often rewards those whose balance is less than 100. Needs to be whitelisted in a channel.
    /guid               Generate a guid.
    /password           Generates a cryptographically-secure password and displays it for all to see.
